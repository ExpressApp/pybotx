from pybotx.domain.ports.async_buffer import AsyncBufferReadable, get_file_size
from pybotx.domain.constants import STICKER_IMAGE_MAX_SIZE
from pybotx.domain.errors import InvalidStickerImageError, StickerImageTooLargeError
...
async def ensure_file_content_is_png(async_buffer: AsyncBufferReadable) -> None:
    ...
    if magic_bytes != PNG_MAGIC_BYTES:
        raise InvalidStickerImageError("Passed file is not PNG")
...
async def ensure_sticker_image_size_valid(async_buffer: AsyncBufferReadable) -> None:
    ...
    if file_size > STICKER_IMAGE_MAX_SIZE:
        ...
        raise StickerImageTooLargeError(
