...
from pybotx.infrastructure.client.exceptions.http import InvalidBotXStatusCodeError
...
def not_found_error_handler(response: HttpResponse) -> NoReturn:
...
class DownloadFileMethod(AuthorizedBotXMethod):
    ...
    async def execute(
        ...
        async with self._botx_method_stream(
            ...
            async for chunk in response.iter_bytes():  # pragma: no branch
