...
@pytest.mark.asyncio
async def test_create_future_without_main_loop_and_set_result_and_wait():
    sync_id = uuid4()
...
@pytest.mark.asyncio
async def test_set_exception_and_wait_for_exception_wrapper():
    sync_id = uuid4()
    wrapper = CallbackWrapper(sync_id)
    wrapper.create_future()
    ...
    with pytest.raises(RuntimeError):
        wrapper._future.result()
    assert wrapper._exception is exc


def test_get_future_before_create_raises_wrapper():
...
@pytest.mark.asyncio
async def test_create_future_with_main_loop_equal_and_set_result_wrapper():
    sync_id = uuid4()
...
def test_create_future_with_different_main_loop_wrapper():
    sync_id = uuid4()
...
def test_create_future_branch_main_loop_not_equal(monkeypatch):
    sync_id = uuid4()
    called = {}
    ...
    wrapper = CallbackWrapper(sync_id, main_loop=fake_loop)
    wrapper.create_future()
...
def test_create_future_exception_branch(monkeypatch):
    sync_id = uuid4()
    called = {}
...
@pytest.mark.asyncio
async def test_set_exception_with_main_loop_equal_wrapper():
    sync_id = uuid4()
    loop = asyncio.get_running_loop()
    wrapper = CallbackWrapper(sync_id, main_loop=loop)
    wrapper.create_future()
...
@pytest.mark.asyncio
async def test_wait_for_result_runtime_error_branch_success_wrapper(monkeypatch):
    sync_id = uuid4()
    wrapper = CallbackWrapper(sync_id)
    wrapper.create_future()
...
@pytest.mark.asyncio
async def test_wait_for_result_runtime_error_branch_exception_wrapper(monkeypatch):
    sync_id = uuid4()
    wrapper = CallbackWrapper(sync_id)
    wrapper.create_future()
...
@pytest.mark.asyncio
async def test_wait_for_result_runtime_error_branch_timeout_wrapper(monkeypatch):
    sync_id = uuid4()
    wrapper = CallbackWrapper(sync_id)
    wrapper.create_future()
...
@pytest.mark.asyncio
async def test_memory_repo_full_cycle():
...
@pytest.mark.asyncio
async def test_wait_timeout_removes_and_raises_callback_not_received():
    repo = CallbackMemoryRepo()
    sync_id = uuid4()
    await repo.create_botx_method_callback(sync_id)
...
@pytest.mark.asyncio
async def test_set_without_create_raises_not_found_repo():
    repo = CallbackMemoryRepo()
    ...
    with pytest.raises(BotXMethodCallbackNotFoundError):
        await repo.set_botx_method_callback_result(Dummy(uuid4()))


@pytest.mark.asyncio
async def test_wait_without_create_raises_not_found_repo():
    repo = CallbackMemoryRepo()
    with pytest.raises(BotXMethodCallbackNotFoundError):
...
@pytest.mark.asyncio
async def test_pop_without_create_raises_keyerror_repo():
    repo = CallbackMemoryRepo()
...
@pytest.mark.asyncio
async def test_stop_callbacks_waiting_sets_exceptions_repo():
    repo = CallbackMemoryRepo()
...
@pytest.mark.asyncio
async def test_get_event_loop_default_and_set_main_loop_repo():
    repo = CallbackMemoryRepo()
...
@pytest.mark.asyncio
async def test_set_result_main_loop_not_equal():
...
@pytest.mark.asyncio
async def test_set_exception_main_loop_not_equal():
...
@pytest.mark.asyncio
async def test_create_future_main_loop_none_after_runtime_error():
...
def test_set_result_runtime_error_handling():
    ...
    def mock_call_soon_threadsafe(fn, *args):
        calls.append((fn, args))
        # Не вызываем fn(*args) чтобы избежать ошибок с future

    main_loop.call_soon_threadsafe = mock_call_soon_threadsafe
    asyncio.get_running_loop = mock_get_running_loop

    try:
        wrapper.set_result("test_result")
        # Проверяем что call_soon_threadsafe был вызван
        assert len(calls) == 1
        assert wrapper._result == "test_result"
...
def test_set_exception_runtime_error_handling():
...
def test_set_result_future_none_branch():
...
def test_set_result_future_done_branch():
    ...
    wrapper._future = asyncio.Future()
    wrapper._future.set_result("already_done")
    ...
    assert wrapper._future.result() == "already_done"


def test_set_exception_future_none_branch():
...
def test_set_exception_future_done_branch():
