...
class Bot:
    def __init__(self, bot_id=None, bot_host=None):
        ...
        if not isinstance(bot_id, str) or not isinstance(bot_host, str):
            raise ValueError(
                "`bot_id` and `bot_host` must be provided and " "must be of str type"
            )

        self.bot_id = bot_id
        self.bot_host = bot_host
        self.url_command = "https://{}/api/v2/botx/command/callback".format(
            self.bot_host
        )
        self.url_notification = "https://{}/api/v2/botx/notification/callback".format(
            self.bot_host
        )
        self.url_file = "https://{}/api/v1/botx/file/callback".format(self.bot_host)
    ...
    def send_message(self, chat_id, text, recipients="all", bubble=None, keyboard=None):
        """
        ...
        if bubble and not isinstance(bubble, ReplyBubbleMarkup):
            raise ValueError(
                "A `bubble` attribute must be of " "`ReplyBubbleMarkup` type"
            )
        ...
        if keyboard and not isinstance(keyboard, ReplyKeyboardMarkup):
            raise ValueError(
                "A `keyboard` attribute must be of " "`ReplyKeyboardMarkup` type"
            )
        ...
        if isinstance(chat_id, SyncID):
            ...
            raise ValueError(
                "`chat_id` must be of type str or list of str, "
